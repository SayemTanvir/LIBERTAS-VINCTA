...
func _monster_checks() -> void:
	...
	var contact_time: float = float(maxi(1, roundi(enemy.sprite.sprite_frames.get_frame_count(enemy.sprite.animation) * 0.42))) / float(enemy.sprite.sprite_frames.get_animation_speed(enemy.sprite.animation))
