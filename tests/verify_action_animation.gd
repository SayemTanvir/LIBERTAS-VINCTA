...
func _monster_checks() -> void:
	...
	var contact_time: float = float(maxi(1, roundi(enemy.sprite.sprite_frames.get_frame_count(enemy.sprite.animation) * 0.25))) / float(enemy.sprite.sprite_frames.get_animation_speed(enemy.sprite.animation))
	check(enemy._strike_pending and FreedomLedger.hp == hp and str(enemy.sprite.animation).begins_with("attack_"), "Contact starts visible windup before damage")
	enemy._update_attack(contact_time * 0.4)
	check(FreedomLedger.hp == hp, "Windup gives a reaction window")
	enemy._update_attack(contact_time * 0.7)
	...
	enemy.stun(1.0)
	enemy._update_attack(contact_time + 0.1)
	check(FreedomLedger.hp == hp and not enemy._strike_pending and str(enemy.sprite.animation).begins_with("stagger_"), "Stun cancels pending damage")
	_reset_attack()
	enemy._resolve_contact()
